...
class amwork_controller extends route
{
	...
	public function index()
	{
       ...
       function fetch_product()
		...
		$msg = array();
		$result = $db->getAllData("SELECT dt.DATE_CREATED, USERNAME, AMOUNT from deposit_tb dt join admin_tb atb on dt.CREATED_BY = atb.ID WHERE date(dt.DATE_CREATED)  >= '$date' AND date(dt.DATE_CREATED) <='$date2' AND CCODE= ".$_COOKIE["ccode"]."  ORDER BY dt.ID DESC LIMIT 1");
		if($result!==0){
		foreach($result as $row)
        {
      	...
      	$result = $db->getAllData("SELECT dt.DATE_CREATED, USERNAME, AMOUNT from withdrawal_tb dt join admin_tb atb on dt.CREATED_BY = atb.ID WHERE date(dt.DATE_CREATED)  >= '$date' AND date(dt.DATE_CREATED) <='$date2' AND CCODE=".$_COOKIE["ccode"]." ORDER BY dt.ID DESC LIMIT 1");
		if($result!==0){
		foreach($result as $row)
        {
