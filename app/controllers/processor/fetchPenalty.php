    ...
    function fetch_loan()
	  {
		...
		$msg = array();
		$result = $db->getAllData("SELECT * from penalty_tb WHERE CCODE=".$_COOKIE["ccode"]."");
		if($result !==0 ){
		foreach($result as $row)
        {
		  ...
		  $msg[] = array("id" => $row["ID"], "amount" => isset($row["AMOUNT"])?number_format($row["AMOUNT"],2):0, "accno"=>$accno, "customer" => $customer, "code"=>$code, "date"=>$row["DATE_NORMAL"], "image"=>$image, "balance"=>isset($row["BALANCE"])?number_format($row["BALANCE"]):0);
        }
