	...
	public function index()
	{
       ...
       function fetch_search()
		...
		if($_SESSION['status'] == '1'){
		  $result = $db->getAllData("SELECT * FROM client_tb WHERE ACCOUNT_STATUS=3 ORDER BY ID DESC");  
		}else{
		   $result = $db->getAllData("SELECT * FROM client_tb WHERE ACCOUNT_STATUS=3 AND POSTED_BY=".$user." ORDER BY ID DESC");  
		}
		if($result!==0){
